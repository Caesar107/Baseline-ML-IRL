...
def analyze_airl_results():
        ...
        for exp_dir in exp_dirs:
            ...
            if os.path.exists(progress_file):
                try:
                    ...
                    if 'AIRL Mean Reward' not in headers:
                        print(f"⚠️  No 'AIRL Mean Reward' column found for {env}")
                        continue
                    
                    mean_reward_idx = headers.index('AIRL Mean Reward')
                    std_reward_idx = headers.index('AIRL Std Reward') if 'AIRL Std Reward' in headers else -1
    ...
    if results:
        ...
        print("\n" + "="*80)
        print("AIRL Training Results Summary")
        print("="*80)
        ...
        # 保存详细结果
        df.to_csv('AIRL/airl_results_detailed.csv', index=False, float_format='%.2f')
        summary_df.to_csv('AIRL/airl_results_summary.csv', index=False)
        
        print(f"\n📁 Detailed results saved to: AIRL/airl_results_detailed.csv")
        print(f"📁 Summary saved to: AIRL/airl_results_summary.csv")
        ...
        print("No AIRL training results found.")
        return None

if __name__ == "__main__":
    analyze_airl_results()
